...
# Check if a custom report directory name is provided
if [[ -n "$1" ]]; then
    report_dir="$1"
    shift  # Remove the first argument (report directory name)
else
    report_dir="reports"
fi
...
if [[ "$1" == "nsys" ]]; then
    ...
    nsys profile --trace=cuda,nvtx,openacc,openmp --stats=true -o "${report_file_base}_nsys" "$@"
    ...
    shift  # Remove the first argument (rtop) and pass the rest to the command
    # Run the ncu command with the specified options
